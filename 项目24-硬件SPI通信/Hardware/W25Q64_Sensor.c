...
void W25Q64_Software_Init(void){
    //初始化W25Q64所需的SPI引脚
    SPI_Software_Init();
}

//指定位置页擦除
void W25Q64_Software_PageErase(uint32_t address){
    SPI_Software_Start();
...
//指定位置写入一个字节
void W25Q64_Software_WriteByte(uint32_t address, uint8_t data){
    SPI_Software_Start();
...
//指定位置读取一个字节
uint8_t W25Q64_Software_ReadByte(uint32_t address){
...
//读取厂商ID和设备ID
void W25Q64_Software_ReadID(uint8_t* manufacturerID, uint16_t* deviceID){
    SPI_Software_Start();

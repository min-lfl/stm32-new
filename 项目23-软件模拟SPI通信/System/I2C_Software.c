...
//发送一个字节，返回应答结果
uint8_t I2C_Software_SendByte(uint8_t byte){
	uint8_t i=0;
	I2C_SCL_PORT -> BRR = I2C_SCL_PIN;  //保证SCL低电平
...
//接收一个字节,ack为是否发送应答
uint8_t I2C_Software_ReceiveByte(uint8_t ack){
	uint8_t byte=0,i=0;
	I2C_SCL_PORT -> BRR = I2C_SCL_PIN;  //保证SCL低电平
	I2C_SDA_PORT -> BSRR = I2C_SDA_PIN;  //把SDA的控制权交给从机
	I2C_Software_Delay();		//这个延迟之后从机应该已经操作了SDA了，所有循环一开始就要马上读取
	for(i=0;i<8;i++){
